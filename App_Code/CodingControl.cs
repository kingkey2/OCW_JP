...
public class CodingControl
{
    ...
    public static string GetEmailTemp(string LoginAccount, string ValidateCode, enumSendMailType SendMailType) {
        ...
        if (SendMailType == enumSendMailType.Register) {
            myStream = new FileStream(HttpContext.Current.Server.MapPath(@"/Html/verification.html"), FileMode.Open);
        } else if (SendMailType == enumSendMailType.ForgetPassword) {
            myStream = new FileStream(HttpContext.Current.Server.MapPath(@"/Html/verification.html"), FileMode.Open);
        } else {
            myStream = new FileStream(HttpContext.Current.Server.MapPath(@"/Html/thanks.html"), FileMode.Open);
        }
    ...
    public static string GetEmailTemp2() {
        ...
        myStream = new FileStream(HttpContext.Current.Server.MapPath(@"/Html/temp1.html"), FileMode.Open);
